...
struct AmbientOcclusionMapFragmentShader
{
  ...
  AmbientOcclusionMapFragmentShader(const Buffer<Real> &depthMap, Buffer<Color> &aoMap) : u_depthMap(depthMap), u_aoMap(aoMap)
  {
  }
...
struct FragmentShader
{
  ...
  GFX::ColorF exec(const varying_type &varying, const std::vector<GFX::Texture> &textures, GFX::vec3 &pixel, bool backFace = false)
  {
    ...
    // scale [0,1] to u_shadowMap size
    Real u = (u_shadowMap.height() - 2) * uv.u; // note: sould work with -1 too, but for some reason there is
    Real v = (u_shadowMap.height() - 2) * uv.v; //       a column of black pixels between each ambient occlussion map

    // interpolate texel color from 4 surrounding texel
    int atom = std::get<4>(varying) + 0.5;
    int offset = atom * u_shadowMap.height(); 
    int u1 = offset + static_cast<int>(u) % u_shadowMap.height();
    int u2 = offset + static_cast<int>(u + 1) % u_shadowMap.height();
    int v1 = static_cast<int>(v) % u_shadowMap.height();
    int v2 = static_cast<int>(v + 1) % u_shadowMap.height();

    const Color &c1 = u_shadowMap(u1, v1);
    const Color &c2 = u_shadowMap(u2, v1);
    const Color &c3 = u_shadowMap(u1, v2);
    const Color &c4 = u_shadowMap(u2, v2);

    Real alpha = u - std::floor(u);
    Real beta = v - std::floor(v);

    Color color1(c1.r * (1.0 - alpha) + c2.r * alpha,
        c1.g * (1.0 - alpha) + c2.g * alpha,
        c1.b * (1.0 - alpha) + c2.b * alpha); 

    Color color2(c3.r * (1.0 - alpha) + c4.r * alpha,
        c3.g * (1.0 - alpha) + c4.g * alpha,
        c3.b * (1.0 - alpha) + c4.b * alpha); 

    Color ao(color1.r * (1.0 - beta) + color2.r * beta,
             color1.g * (1.0 - beta) + color2.g * beta,
             color1.b * (1.0 - beta) + color2.b * beta); 

    //return ao;

    Real accessibility = ao.r / 255.0;
...
void mirror_texels(Buffer<Color> &buffer)
{
...
PDBWidget::PDBWidget(const std::string &filename, int width, int height, QWidget *parent) : GfxWidget(width, height, parent)
{
  setEyeZ(1);
  ...
  */
  ...
  m_aoMap = Buffer<Color>(AO_MAP_SIZE * atoms.size(), AO_MAP_SIZE);
  ...
  for (int i = 0; i < m_aoMap.width(); ++i)
    for (int j = 0; j < m_aoMap.height(); ++j) {
      int ao = m_aoMap(i, j).b * factor;
      m_aoMap(i, j) = Color(ao, ao, ao);
    }
...
void PDBWidget::render()
{
  ...
  //context().disable(GFX_BUFFER, GFX_COLOR_BUFFER);
  ...
  fragmentShader.u_shadowMap = m_aoMap;
